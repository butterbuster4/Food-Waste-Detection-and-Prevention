...
import com.plt.grp05h5.domain.SessionVariables;
...
public class GlobalContext {
    ...
    */
    ...
    public static void fillInCurrentUserInSessionVariable(String userId,String userName) {
        fillInCurrentUserInSessionVariable(userId, userName,null, null);
    }

    public static void fillInUserInfoInSessionVariable(UserContext.UserInfo userInfo) {
        if(Objects.nonNull(userInfo.extraInfo)){
            Map<String,String> extraMap = null;
            try{
                extraMap = JacksonUtils.fromJson(userInfo.extraInfo, Map.class);
            }catch (Exception e){
                LOGGER.error("extraInfo is not json format, userId={}", userInfo.userId);
            }
            GlobalContext.fillInCurrentUserInSessionVariable(userInfo.userId,userInfo.userName,userInfo.nickName,extraMap);
        }else{
            GlobalContext.fillInCurrentUserInSessionVariable(userInfo.userId,userInfo.userName,userInfo.nickName,null);
        }
    }

    public static void fillInCurrentUserInSessionVariable(String userId
            ...
            try {
                ...
                if (field.getType().getName().equals("com.netease.lowcode.auth.domain.LCAPUser")  && "currentUser".equalsIgnoreCase(field.getName())) {
                    ...
                    if(Objects.nonNull(displayName)){
                        Field displayNameFiled = ReflectionUtils.findField(field.getType(), "displayName", String.class);
                        if (Objects.nonNull(displayNameFiled)) {
                            ReflectionUtils.makeAccessible(displayNameFiled);
                            ReflectionUtils.setField(displayNameFiled, ReflectionUtils.getField(field,sessionVariables), displayName);
                        }
                    }
                }
            } catch (Exception e) {
