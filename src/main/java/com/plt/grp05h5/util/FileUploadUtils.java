...
import static com.netease.cloud.codewave.file.connector.CodeWaveFileConstants.DOWNLOAD_FORM_ORIGINAL_S3_CLIENT;
...
public final class FileUploadUtils {
    ...
    public static boolean shouldAccessControl(String apiPath) {
        if (StringUtils.isNotBlank(apiPath) && getPathPermissionSuffix(apiPath)
                .startsWith(Constants.UPLOAD_ACCESS_SUFFIX_MAGIC + Constants.UPLOAD_ACCESS_PRIVATE_SUFFIX)) {
            return true;
        }
        //新版本
        CodeWaveFileUrl codeWaveFileUrl = CodeWaveFileUrl.fromUri(apiPath);
        if (codeWaveFileUrl.contains(CodeWaveFileConstants.FILE_ACCESS)
                && codeWaveFileUrl.getQueryStringValue(CodeWaveFileConstants.FILE_ACCESS).equals(Constants.UPLOAD_ACCESS_PRIVATE_SUFFIX)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isPathExpiration(String apiPath) {
        //新版本
        CodeWaveFileUrl codeWaveFileUrl = CodeWaveFileUrl.fromUri(apiPath);
        if (codeWaveFileUrl.contains(CodeWaveFileConstants.EXPIRE_TIME)) {
            long expirationTime = Long.parseLong(codeWaveFileUrl.getQueryStringValue(CodeWaveFileConstants.EXPIRE_TIME));
