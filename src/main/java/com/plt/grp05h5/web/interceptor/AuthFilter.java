...
import com.plt.grp05h5.service.system.UserServiceFactory;
import com.plt.grp05h5.util.FileUploadUtils;
...
public class AuthFilter implements Filter {
    ...
    private void processNotLoginResponse(String path, ServletResponse servletResponse) throws IOException {
        if (Constants.SYSTEM_AUTHENTICATED_SPECIAL_APIS.stream().anyMatch(s -> path.startsWith(s))) {
            ...
            response.setContentType(Constants.AUTH_FILTER_HEADER_CONTENT_TYPE);
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            String message = messageSource.getMessage(ErrorCodeEnum.NO_PERMISSION_ACCESS_RESOURCE.code,null, ErrorCodeEnum.NO_PERMISSION_ACCESS_RESOURCE.code, LocaleContextHolder.getLocale());
            ApiReturn<Object> apiReturn = ApiReturn.ofError(HttpStatus.UNAUTHORIZED.value(),ErrorManager.ofAuthUnAuthorizedError(message),message);
            response.getWriter().write(objectMapper.writeValueAsString(apiReturn));
        }
    }
