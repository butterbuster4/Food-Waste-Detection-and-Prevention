...
import com.plt.grp05h5.iam.auth.AuthService;
import com.plt.grp05h5.util.GlobalContext;
...
public class UserContextFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(matchedAuthServiceList)) {
                for (int i = 0; i < matchedAuthServiceList.size(); i++) {
                    ...
                    if (Objects.nonNull(UserContext.getCurrentUser())) {
                        GlobalContext.fillInUserInfoInSessionVariable((UserContext.getCurrentUser()));
            ...
            log.error("userContextFilter handle exception:{}", e);
        }
        try {
            chain.doFilter(request, servletResponse);
        } finally {
